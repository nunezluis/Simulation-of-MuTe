...
%%%%%%% SUBSECTION %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experimental results from the scintillator detector}\label{sec:bar-data}%Rolando
...
Since the \textsl{SiPMs} have intrinsic noise, then it is important to know well the detector, in order to define a methodology with a discriminant to detect particles and correctly interpret the measured data. A first study was made to guarantee that the \textsl{SiPM} keeps working in Geiger mode at any temperature. Section \ref{breakdown-voltage} presents the results of the dependence of the breakdown voltage with the temperature.

On the other hand, given that the scintillator, the fiber and the \textsl{SiPM} are not $100$\% efficient, there are various parameters to take into account. For example, the material of the bars is opaque to photons with an average length of $5.5$cm of attenuation. Then, in order to have a detector of $120$cm length, a \textsl{WLS} fiber is placed inside the bar. The fibers guide the photons, but they can present attenuation in spite of being multi-layer, i.e. some photons can escape from the guide and even be reflected in the border. This represents an additional problem when coupling different scintillators, and there will be edge effects related to the refractive index of each one. Therefore, we must quantify the attenuation in these fibers and determine whether the coupling and the attenuation will be relevant in the case of the MuTe scintillator bars. A controlled experiment was performed to estimate the percentage of attenuation in the system (see section \ref{attenuation-experimental}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% SUBSECTION %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Dependence of the breakdown voltage with the temperature}\label{breakdown-voltage}%Rolando
To study the response of each scintillator bar, we first studied the stability of the \textsl{SiPMs} with temperature changes. \textsl{SiPMs} can change their mode of operation if the input voltage ($V_{Bias}$) is less than the breakdown voltage ($V_B$). $V_B$ varies if the \textsl{SiPM} temperature changes. To solve this problem, a relationship must be found between the $V_B$ and the temperature at which the \textsl{SiPM} works. With this relationship we can obtain an adjustment parameter to ensure that \textsl{SiPMs} always work in Geiger mode.

For this purpose, a temperature controlled box was built. This box was regulated by a TEC1-12706 thermoelectric peltier, mounted on an aluminum frame where we located the mechanical coupling of the \textsl{SiPM}. This structure was introduced in a thermally insulated box. The control system consist of two sensors: one to vary the temperature and the other one to measure the temperature of the \textsl{SiPM}. For different target temperatures ($10^{\circ}$C, $20^{\circ}$C, $40^{\circ}$C and $50^{\circ}$C) the system set-up time was measured. From the figure \ref{timesetup}, it is noted that the time required to reach the target temperature, in each case, is around $(600 \pm 50)$s.

\begin{figure}[h!] %Rocal
    \centering
        \includegraphics[scale=0.48]{Figures/tiempo_establecemiento.png}
   \caption{Measurements of the system set-up time with different target temperatures: $10^{\circ}$C, $20^{\circ}$C, $40^{\circ}$C and $50^{\circ}$C. We can see that the time required to reach the target temperature in each case is around $(600 \pm 50)$s. }\label{timesetup}
\end{figure}

After this test, the \textsl{SiPM} power supply was mounted and the DarkCurrent \cite{Renker2006} was measured in the range of $40$V to $60$V. This was done for $5$ target temperatures: $0^{\circ}$C, $10^{\circ}$C, $20^{\circ}$C, $30^{\circ}$C, $40^{\circ}$C and $50^{\circ}$C. The breakdown voltage was determined from each case and the relationship with the temperature is in figure \ref{temperature}. From this graph we have that for each $10^{\circ}$C of temperature, the $V_B$ changes around $0.45$V.
...
In order to calculate the attenuation in the scintillator bars, measurements were taken of the signal produced by the passage of charged particles in three positions of a test bar: at the end where the \textsl{SiPM} is located, in the middle and at the opposite end. To generate the trigger signal, small scintillators with rectangular cross-section and dimensions ($4$cm $\times$ $4$cm $\times$ $1$cm) were used. These also have a \textsl{WLS} fiber-\textsl{SiPM} coupling system. This assembly of bars in coincidence was carried out inside a black box. The scintillators were placed on a frame that slid on a rail along the test bar as shown in figure \ref{coincidencia_barras}. Then, an event occurs when there is a signal in the small scintillator on top of the test bar, there is a signal on the test bar and then on the small scintillator below it.  

\begin{figure}[h!] %Rocal
    \centering
        \includegraphics[scale=0.23]{Figures/barras_coincidencia.png}
   \caption{Diagram of the experimental set-up, assembly and dimensions of the system in coincidence of 3 plastic scintillators to measure the attenuation of the signal produced in the \textsl{SiPM} to the passage of charged particles. The 3 positions of interest are shown on the test bar: at the end where the \textsl{SiPM} is located, in the middle and at the opposite end. } \label{coincidencia_barras}
\end{figure}

The trigger system was connected to a RedPitaya development card capable of recording pulses with a frequency of 125 Mhz \cite{Pitaya2016}. The system was synchronized to have pulses in coincidence, having three input signals to the card and one output signal that correspond to each position in the scintillator test bar. The coinciding system had an angle $\theta$ for this configuration of $106.26^{\circ}$ as shown in the figure \ref{coincidencia_barras}. The frequency of events per minute that was measured experimentally with the aid of an oscilloscope was $10\pm1$ per minute for $16$cm$^2$. With these data was estimate that were detected 0.62 particles$\times$min$\times$cm$^2$. 

After performing a calibration with respect to the noise, $10000$ reference pulses were taken in the three positions of interest. In the position opposite to \textsl{SiPM}, the fiber was cut to $45^{\circ}$ to maximize photon leakage and avoid secondary pulses by reflections at the end of the fiber. From each average pulse we calculate the mean charge deposited in the three positions, to estimate the percentage of attenuation by normalizing and comparing the values. The results shown in figure \ref{experimental_attenuation}, where an exponential adjustment coincides with the attenuated signal. The function used was $f(x)=0.88+a*exp(-x*b)$ with the parameters $a$=0.126652, $b$= 0.0205653 with an error of +/- 0.003444 (2.72\%) and +/- 0.001065 (5.18\%) respectively.

In this case the difference between the mean charge value deposited at the far end of the \textsl{SiPM} and the nearest is around 11\%. That is, if the average charge deposited by a muon or electron near the \textsl{SiPM} is 1, the average charge deposited by the same particle at the farthest opposite end is 11\% less and represents the maximum attenuation in this 120 cm long active detection system.

\begin{figure}[h!] %Rocal
    \centering
        \includegraphics[scale=0.49]{Figures/atenuacion_esperimental.png}
   \caption{Porcentual attenuation of the signal in the test bar obtained with the system in coincidence of 3 plastic scintillators. The maximum amplitude is in the position closest to the SiPM, which then decreases by 11\% at the opposite end. Three positions were obtained with data and some intermediate points come from the fit made, which corresponds to an exponential function. } \label{experimental_attenuation}
\end{figure}
...
%%%%%%% SUBSECTION %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{First WCD measurements}\label{sec:wcd-data}%Jesus
